...
public function update_clients(){
    ...
    //$edit_media_description=$this->input->post('editor13');
	 ...
	 $update_clients_list=$this->all_admin_model->update_clients($edit_clients_title,$edit_clients_address,$edit_position,$current_position,$edit_clients_slno);
...
public function add_image_type_category(){
	...
	$this->load->view('admin/add_image_category');
...
$show_all_icategory['icategory_show']=$this->all_admin_model->show_all_category();

$this->load->view('admin/all_image_ategory',$show_all_icategory);

}

public function add_icategory_code(){

	$image_category_name=$this->input->post('category_name');
 	...
 	$show_all_icategory['icategory_show']=$this->all_admin_model->show_all_category();
	$show_all_icategory['category_susscess']='<font style="color:green;">Category Added Successfully.</font>';

		$this->load->view('admin/all_image_ategory',$show_all_icategory);
 }
}

public function edit_image_category(){

	$image_cat_slno=$this->uri->segment(2);
	$edit_image_category['edit_image_cat_list']=$this->all_admin_model->edit_imag_cat($image_cat_slno);
	$this->load->view('admin/edit_icategory',$edit_image_category);

	}

public function edit_icategory_code(){


	$edit_img_cat_slno=$this->input->post('edit_imag_cat_slno');

	$edit_img_cat_name=$this->input->post('edit_cat_name');

	$update_img_cat_list=$this->all_admin_model->update_image_category_name($edit_img_cat_name,$edit_img_cat_slno);

	if($update_img_cat_list){

		$show_all_icategory['icategory_show']=$this->all_admin_model->show_all_category();
		$show_all_icategory['icategory_msg']='<font style="color:green;">Category Updated Successfully.</font>';
...
public function delete_icategory(){

	$delete_icat_slno=$this->input->post('new_del_icat');
  	$this->all_admin_model->delete_image_category($delete_icat_slno);
}
    ...
    $filename7='';
     if (empty($_FILES['type_image']['name']) || empty($_FILES['type_image_new']['name'])){
	   ...
	   $this->all_admin_model->add_type_image($content_type,$image_title,$type_description,$filename7,$filename13);
	    ...
	    $show_all_type['type']=$this->all_admin_model->show_all_type($content_type);
	    $show_all_type['type_susscess']='<font style="color:green;">Type Image Added Successfully.</font>';
	    $show_all_type['selected_cat']=$content_type;
...
public function all_type_image(){
	/*echo $this->input->post('typeval');
	exit;*/
	$select_cat=$this->input->post('typeval');
	if(!empty($select_cat))
	 {
		$show_all_type=$this->all_admin_model->show_all_type($select_cat);
		echo json_encode($show_all_type);
	 }
	else
	{
		$select_cat=$this->input->post('display_cat');
	
	 	$show_all_type['type']=$this->all_admin_model->show_all_type($select_cat);
	    $this->load->view('admin/all_type_image',$show_all_type);
	 }
}
...
public function update_type_code(){
    ...
    $select_cat=$this->input->post('edit_type_cat');
	 ...
	 if($update_type_list){
        ...
        $show_all_type['selected_cat']=$select_cat;
...
public function add_type_video_code(){
    ...
    $filename8='';
     if (empty($_FILES['type_video_image']['name'])){
	    ...
	    $filename8=do_upload_new($filedata8);
...
public function logout(){
	...
	public function all_category(){
		...
		$all_video_category['category_show']=$this->all_admin_model->show_all_vcategory();
		$this->load->view('admin/all_category',$all_video_category);
	}


	public function add_vcategory_code(){

		$vcategory_name=$this->input->post('category_name');
		...
		$this->all_admin_model->add_video_category($vcategory_name);

		$show_all_category['category_show']=$this->all_admin_model->show_all_vcategory();
	    $show_all_category['category_susscess']='<font style="color:green;">Category Added Successfully.</font>';

		$this->load->view('admin/all_Category',$show_all_category);
	}

	}

	public function edit_video_category(){

	$video_slno=$this->uri->segment(2);
	$edit_video_category['edit_video_list']=$this->all_admin_model->edit_video($video_slno);
	$this->load->view('admin/edit_vcategory',$edit_video_category);
}


public function edit_vcategory_code(){

	$edit_cat_name=$this->input->post('edit_cat_name');
	$edit_cat_slno=$this->input->post('edit_video_slno');

	$update_video_cat_list=$this->all_admin_model->update_category_name($edit_cat_name,$edit_cat_slno);

	if($update_video_cat_list){

		$show_all_category['category_show']=$this->all_admin_model->show_all_vcategory();
		$show_all_category['category_msg']='<font style="color:green;">Category Updated Successfully.</font>';
	 	
	   $this->load->view('admin/all_Category',$show_all_category);
	}
}

public function delete_vcategory(){


    $delete_vcat_slno=$this->input->post('new_del_vcat');
  	$this->all_admin_model->delete_video_category($delete_vcat_slno);
}


public function delete_selected_type_image(){

	  $ids = $this->input->post('ids');

	  $fetch_selected_image=$this->db->query("select *,GROUP_CONCAT(small_image SEPARATOR ',') images,GROUP_CONCAT(image SEPARATOR ',') bimages from tbl_type_image where slno in(".$ids.")");

	  $all_selected_image=$fetch_selected_image->row();

	  $as_images=$all_selected_image->images;
	  $ab_images=$all_selected_image->bimages;

       $unlink_small_images=explode(',',$as_images);
       $unlink_big_images=explode(',',$ab_images);


       foreach($unlink_small_images as $value1){
	   ...
	   foreach($unlink_big_images as $value2){
         ...
         $this->db->where_in('slno', explode(",", $ids));
         $this->db->delete('tbl_type_image');
