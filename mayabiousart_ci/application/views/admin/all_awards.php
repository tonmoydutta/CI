...
<div class="content-wrapper" style="min-height: ">
    ...
    <?php if(!empty($awards_susscess)){?>
        <?=$awards_susscess;
       }
        ?>


        <?php if(!empty($awards_msg)){?>
        <?=$awards_msg;
       }
        ?>
     <?php if(!empty($delete_news_msg)){?>
      ...
      <ol class="breadcrumb">
        <li><a href="<?=base_url();?>add_new_awards" style="padding:3px 13px 3px; background-color:#00b33c; color: #fff; font-weight: bold;">Add New</a></li>
...
?>
                ...
                <tr id="award_tr_<?=$row->slno;?>">
                  <td><?=$i;?></td>
                  <td><?=$row->title;?></td>
                  
                  <td><?php echo substr($row->description,0,60);?></td>
                  <td><img src="uploads/<?=$row->image;?>" height="50px;" width="60px;"></td>
                  <td><a href="<?=base_url();?>edit_awards/<?=$row->slno;?>">Edit</a> | <a href="#" onclick="delete_awards(<?=$row->slno;?>);">Delete</a></td>
